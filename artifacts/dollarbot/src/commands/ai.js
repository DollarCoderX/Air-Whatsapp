...
async function groqRequest(body, maxRetries = 2) {
  for (let i = 0; i < maxRetries; i++) {
    ...
    try {
      // Prepare a sanitized copy of the body to avoid provider-specific
      // validation errors (e.g. Groq rejecting unknown properties like `ts`).
      const sendBody = JSON.parse(JSON.stringify(body));

      // Recursively remove any disallowed keys that may appear anywhere
      // in the payload (top-level or nested). This guards against client
      // frameworks injecting timestamps/ids into message objects.
      ...
      deepRemoveKeys(sendBody, ['ts', 'id', 'messageTimestamp', 'key']);
...
3. In ALL normal, respectful conversations: NEVER use these tokens. Do not mention them.` +
      ...
      if (command) {
        await aiBridge.runBridgedCommand(sock, msg, jid, command);
        if (command === 'menu') return; // bridged command handles sending the menu
      }
  ...
  async mera(sock, msg, args, jid) {
    ...
    try {
      ...
      if (command) {
        await aiBridge.runBridgedCommand(sock, msg, jid, command);
        if (command === 'menu') return; // bridged command will send the menu
      }
      ...
      if (searchRes.ok) {
        ...
        if (manga) {
          ...
          const text =
            ...
            `┃ ${synopsis.replace(/\n/g, '\n┃ ')}\n` +
            `╰━━━━━━━━━━━━━━━━━━⬣\n\n` +
            `_🔗 MyAnimeList · Powered by Air Bot_`;
  ...
  // ── .air — Air 4.3 model from the AIR INTELLIGENCE family
  async air(sock, msg, args, jid) {
    if (!args.length) return sock.sendMessage(jid, { text: '╭━━━〔 Air 4.3 〕━━━⬣\n┃ Usage: .air <your question>\n╰━━━━━━━━━━━━━━━━━━⬣' }, { quoted: msg });
    const prompt = args.join(' ');
    await sock.sendMessage(jid, { text: '_Thinking with Air 4.3..._' }, { quoted: msg });
    try {
      const res = await groqRequest({
        model: 'air-4.3',
        messages: [
          { role: 'system', content: 'You are Air 4.3, a model from the AIR INTELLIGENCE family created by Shapes. AIR includes Air IV, Air 4.3, Air 4.5, and Air Imagine. Answer clearly, concisely, and naturally using WhatsApp markdown when appropriate.' },
          { role: 'user', content: prompt },
        ],
      });
      await sock.sendMessage(jid, { text: `╭━━━〔 Air 4.3 〕━━━⬣\n\n${res}\n\n╰━━━━━━━━━━━━━━━━━━⬣` }, { quoted: msg });
    } catch (e) { await sock.sendMessage(jid, { text: `Air Error: ${e.message}` }, { quoted: msg }); }
  },
};

// ── V6 AI Models — only the five approved chatbots ────────────────

const V6_MODELS = {
  chatgpt: {
    model: 'openai',

    system: `You are ChatGPT, a highly capable assistant. Act like a natural, polished ChatGPT assistant: helpful, clear, concise, and conversational. Use WhatsApp markdown only: *bold* for key ideas, _italic_ for emphasis, simple bullets when useful. Keep responses practical and human-sounding.`
  },
  claude: {
    model: 'claude',
    system: `You are Claude, an analytical and thoughtful assistant. Speak with calm clarity, use practical reasoning, and answer directly. Use WhatsApp markdown only: *bold* for important points, _italic_ for emphasis, and short lists when helpful.`
  },
  mistral: {
    model: 'mistral',
    system: `You are Mistral, a capable and efficient AI assistant. Keep replies clear, actionable, and well-structured. Use WhatsApp markdown only: *bold* for key concepts, _italic_ for emphasis, and simple bullets when helpful.`
  },
  meta: {
    model: 'meta',
    system: `You are Meta, a modern AI assistant with strong reasoning and helpful communication. Speak clearly, respectfully, and efficiently. Use WhatsApp markdown only: *bold* for key ideas, _italic_ for emphasis, and concise lists when useful.`
  },
  air: {
    model: 'air-4.3',
    system: `You are Air 4.3, an AI model created by Shapes and part of the AIR INTELLIGENCE family. AIR includes Air IV, Air 4.3, Air 4.5, and Air Imagine. Respond clearly, naturally, and efficiently using WhatsApp markdown only: *bold* for important ideas, _italic_ for emphasis, and bullets when useful.`
  },
};
...
aiCommands.ChatGpt = aiCommands.chatgpt;
aiCommands.claude = (sock, msg, args, jid) => v6Chat(sock, msg, args, jid, 'claude');
aiCommands.mistral = (sock, msg, args, jid) => v6Chat(sock, msg, args, jid, 'mistral');
aiCommands.meta = (sock, msg, args, jid) => v6Chat(sock, msg, args, jid, 'meta');
aiCommands.air = (sock, msg, args, jid) => v6Chat(sock, msg, args, jid, 'air');
