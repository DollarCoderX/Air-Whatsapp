...
const creativeCommands = {
  ...
  async sonnet(sock, msg, args) {
    ...
    await send(sock, msg.key.remoteJid, msg, box('📜 SONNET', r, '_Written by Air Bot_'));
  },
  ...
  async jingle(sock, msg, args) {
    const product = args.join(' ') || 'Air Bot';
  ...
  async productdesc(sock, msg, args) {
    const product = args.join(' ') || 'Air Premium';
