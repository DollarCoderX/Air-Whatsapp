...
async function sendOtpEmail(toEmail, otp) {
  ...
  const res = await fetch('https://api.brevo.com/v3/smtp/email', {
    ...
    body: JSON.stringify({
      sender: { email: 'uraincle@gmail.com', name: 'Air Bot' },
      to: [{ email: toEmail }],
      subject: '🤖 Your Air Bot Verification Code',
      htmlContent: `
        <div style="font-family: Arial, sans-serif; max-width: 520px; margin: 0 auto; background: #f9f9f9; padding: 32px; border-radius: 12px;">
          <h2 style="color: #111; margin-bottom: 4px;">🤖 Air Bot Verification</h2>
          ...
          <p style="color: #aaa; font-size: 12px;">Sent by Air Bot — powered by Air Engine</p>
        </div>`,
    }),
  });
