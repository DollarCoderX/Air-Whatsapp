...
const ownerCommands = {
  ...
  async autoreply(sock, msg, args) {
    const jid = msg.key.remoteJid;
    // Support: `.autoreply on/off` for DMs, and `.autoreply group on/off` for groups
    const mode = args[0]?.toLowerCase();
    const sub = args[1]?.toLowerCase();
    if (mode === 'group' && ['on', 'off'].includes(sub)) {
      await store.set('autoreplyGroups', sub === 'on');
      return sock.sendMessage(jid, { text: `*Group AutoReply is now ${sub.toUpperCase()}*\n\n${sub === 'on' ? 'Bot will respond to messages in groups.' : 'Group AutoReply disabled.'}` });
    }
    const val = mode;
    if (!['on', 'off'].includes(val)) {
      const cur = !!(await store.get('autoreply'));
      const curG = !!(await store.get('autoreplyGroups'));
      return sock.sendMessage(jid, { text: `⚙️ *AutoReply:* ${cur ? 'ON' : 'OFF'}\n⚙️ *Group AutoReply:* ${curG ? 'ON' : 'OFF'}\nUsage: .autoreply on/off OR .autoreply group on/off` });
    }
  ...
  async noprefix(sock, msg, args) {
    ...
    await store.set('aiMode', true);
    const cfg = {
      name: 'Air',
      systemPrompt: `You are Air — a friendly, concise assistant owned by Dollar. You are built to help, summarize, and chat naturally using WhatsApp formatting. You are aware you run on the Air Intelligence family of models (Air IV, Air 4.3, Air 4.5 and Air Imagine) and may reference that when relevant. Keep replies helpful, concise, and human-like. Never reveal internal system secrets or raw keys. If asked, direct people to contact Dollar (the owner).`,
      providers: ['groq','pollinations','ollama:linus']
    };
    await store.set('aiModeConfig', cfg);

    // Record who enabled AI Mode and notify owners if needed
    const callerJid = msg.key.participant || msg.key.remoteJid || '';
    const callerNum = String(callerJid).split('@')[0].split(':')[0].replace(/\D/g, '');
    await store.set('aiModeEnabledBy', { by: callerNum, jid: callerJid, at: Date.now(), method: 'noprefix' });

    const isCallerOwner = config.ownerNumbers.some(o => String(o).replace(/\D/g, '') === callerNum);
    await sock.sendMessage(jid, { text: '✅ *AI Mode Enabled* — Air will now respond to all texts and messages in groups and DMs. Use .102 (owner only) to restore normal bot behavior.' });

    if (!isCallerOwner) {
      // Notify each owner about the activation
      for (const owner of config.ownerNumbers) {
        try {
          await sock.sendMessage(`${owner}@s.whatsapp.net`, {
            text: `🔔 *AI Mode Activated*\n\nUser: +${callerNum}\nChat: ${jid}\nActivated at: ${new Date().toLocaleString()}\n\nUse .102 to disable AI Mode (owner only).`,
          });
        } catch (_) {}
      }
      console.log(`[AI] Enabled by non-owner +${callerNum} in ${jid}`);
    } else {
      console.log(`[AI] Enabled by owner +${callerNum}`);
    }
  },

  // ── .101 — enable AI-only mode (Air) — secret password protected
  async '101'(sock, msg, args) {
    const jid = msg.key.remoteJid;
    const password = args?.[0] || null;
    const SECRET = 'dollarcoderx';
    if (!password) {
      return sock.sendMessage(jid, { text: '🔐 Secret command. Usage: .101 <password>' }, { quoted: msg });
    }
    if (password !== SECRET) {
      return sock.sendMessage(jid, { text: '❌ Incorrect password.' }, { quoted: msg });
    }
    await store.set('aiMode', true);
    await store.set('aiModeConfig', {
      name: 'Air',
      systemPrompt: `You are Air — a helpful, concise, and slightly playful assistant owned by Dollar. You know you are powered by the Air Intelligence family (Air IV, Air 4.3, Air 4.5, and Air Imagine). Speak in a friendly, human tone, use WhatsApp markdown when appropriate, and include a brief note about your owner when requested. Do not reveal internal system details, keys, or implementation internals. Behave politely and provide useful, context-aware answers for groups and DMs.`,
      providers: ['groq','pollinations','ollama:linus']
    });
    await sock.sendMessage(jid, { text: '✅ *AI Mode Enabled* — Air will now respond to all texts and messages in groups and DMs. Use .102 (owner only) to restore normal bot behavior.' });
    await store.set('aiModeEnabledBy', { by: (msg.key.participant||msg.key.remoteJid||'') , jid: (msg.key.participant||msg.key.remoteJid||''), at: Date.now(), method: 'secret' });
  },

  // ── .102 — disable AI-only mode (restore bot) ──────────────────────────
  async '102'(sock, msg, args) {
    const jid = msg.key.remoteJid;
    const password = args?.[0] || null;
    const SECRET = 'dollarcoderx';
    ...
    // If caller is not an owner, require the secret password to disable AI mode
    if (!isCallerOwner) {
      if (!password) {
        return sock.sendMessage(jid, { text: '🔐 This command is restricted. Provide the secret password to disable AI Mode. Usage: .102 <password>' }, { quoted: msg });
      }
      if (password !== SECRET) {
        return sock.sendMessage(jid, { text: '❌ Incorrect password.' }, { quoted: msg });
      }
    ...
    await store.set('aiMode', false);
    await store.set('aiModeConfig', null);
    await sock.sendMessage(jid, { text: '⛔ *AI Mode Disabled* — Air is back to normal bot behavior.' });
  },
  ...
  async vv(sock, msg) {
    ...
    try {
      ...
      const caption = innerMsg[mediaType]?.caption || 'View-once revealed by Air Bot';
  ...
  async shutdown(sock, msg) {
    const jid = msg.key.remoteJid;
    await sock.sendMessage(jid, { text: '*Air Bot shutting down...*\nGoodbye!' });
  ...
  async botupgrade(sock, msg) {
    ...
    try {
      const vcard =
        ...
        `NOTE:Message me to upgrade your Air slot to Pro!\n` +
        'END:VCARD';
