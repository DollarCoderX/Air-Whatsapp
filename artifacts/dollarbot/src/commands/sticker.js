 ...
 *   .steal     — re-brand any replied sticker with Air name + 🇨🇦 flag
 *   .steal     — re-brand any replied sticker with Air Bot name + 🇨🇦 flag
 */
...
const stickerCommands = {
  ...
  async sticker(sock, msg, args) {
    ...
    try {
      ...
      const parts    = args.join(' ').split('|');
        const packname = parts[0]?.trim() || 'Air Bot 💵';
        const author   = parts[1]?.trim() || 'Air 🇨🇦';
  ...
  async toimage(sock, msg) {
    ...
    try {
      ...
      await sock.sendMessage(jid, { image: imgBuf, caption: '_Sticker converted to image by Air Bot_ 💵' }, { quoted: msg });
    } catch (e) {
  ...
  //    freshly-branded sticker) with Air identity ────────────────────
    ...
    try {
      ...
      if (!media) {
        return msg.reply(
          ...
          '_This will re-brand it with Air Bot\'s name and save it to your collection._'
        );
      }
  ...
  async savestatus(sock, msg) {
    ...
    try {
      ...
      if (mediaType === 'imageMessage') {
        await sock.sendMessage(jid, { image: buffer, caption: '✅ *Saved!* — Air Bot 💵' }, { quoted: msg });
      } else if (mediaType === 'videoMessage') {
        await sock.sendMessage(jid, { video: buffer, caption: '✅ *Saved!* — Air Bot 💵' }, { quoted: msg });
  ...
  async xemoji(sock, msg, args) {
    ...
    try {
      const stickerBuf = await imageToSticker(imgBuf, 'Air Bot 💵', 'Air 🇨🇦');
      await sock.sendMessage(jid, { sticker: stickerBuf }, { quoted: msg });
    } catch (e) {
  ...
  async attp(sock, msg, args) {
    ...
    try {
      ...
      const stickerBuf = await imageToSticker(buf, 'Air Bot 🇨🇦', 'Air');
      await sock.sendMessage(jid, { sticker: stickerBuf }, { quoted: msg });
    } catch (e) {
