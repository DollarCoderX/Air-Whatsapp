...
const dashboardBus = (() => { try { return require('./lib/dashboardBus'); } catch { return null; } })();
...
function isOwnerJid(jid) {
  if (!jid) return false;
  // Extract the numeric part of the JID (handles device suffixes and weird formats)
  const raw = String(jid).split('@')[0].split(':')[0];
  const digits = raw.replace(/\D/g, '');
  if (!digits) return false;
  return config.ownerNumbers.some(o => String(o).replace(/\D/g, '') === digits || (jid || '').includes(String(o)));
}
...
const CMD_EMOJIS = {
  ...
  bypass:'🔓',
  // AI – AirFive Models
  chatgpt:'🤖', claude:'🧠', mistral:'⚡', meta:'🌐', air:'✨',
...
// AI typing/generation state to avoid spamming and enable human-like delays
const aiTypingState = { generating: {}, lastReplyAt: {} };
...
async function sendMenu(sock, jid, speedMs, quotedMsg, holiday) {
  ...
  // Remove a recently-sent duplicate textual menu (if present).
  // Some AI flows or retries may have already posted the menu text
  // before media arrived; delete that bot message to avoid a text
  // message appearing prior to the video/image menu.
  try {
    const chatArr = global.msgStore?.messages?.[jid]?.array;
    if (Array.isArray(chatArr)) {
      for (let i = chatArr.length - 1; i >= 0; i--) {
        const m = chatArr[i];
        if (!m || !m.key) continue;
        if (!m.key.fromMe) continue;
        const c = m.message?.conversation || m.message?.extendedTextMessage?.text || '';
        if (!c) continue;
        // Heuristic: menu texts contain the bot title or the prompt to use .menu
        if (c.includes('Air') || c.includes('menu') || c.includes('Use .menu')) {
  ...
  try {
    ...
    // Never send an automatic plain-text fallback here. The media send may
    // still complete later and sending a text fallback causes duplicate
    // menus. Log and move on.
    try { console.error('[sendMenu] media send failed:', err?.message || err); } catch (_) {}
  }
  ...
  const e2  = th ? th.e2  : '☠️';
  const div = th ? th.div : '◇━━━━━━━━━━━━━━━━━━━━━━━━━◇';
  const titleLine = th ? th.title : `☠️ *Air* ☠️`;
  const subLine   = th ? `\n${th.sub}` : '';
  const footerLine = th ? th.footer : `✦ *Air— AIR Intelligence* ✦`;
  ...
  try {
    ...
    if (err && /timeout/i.test(err.message)) {
      // skip text fallback to avoid duplicate messages
    } else {
      await safeSend(sock, jid, { text: caption }, replyOptions(quotedMsg));
    }
  }
...
async function handleMessage(sock, msg) {
  try {
    ...
    switch (cmd) {
      ...
      case '101':        await ownerCommands['101'](sock, msg, args); break;
      case '102':        // allow .102 to reach the command handler so it can accept a password for non-owners
              await ownerCommands['102'](sock, msg, args); break;
      ...
      case 'claude':  await aiCommands.claude(sock, msg, args, jid); break;
      case 'mistral': await aiCommands.mistral(sock, msg, args, jid); break;
      case 'meta':    await aiCommands.meta(sock, msg, args, jid); break;
      case 'air':     await aiCommands.air(sock, msg, args, jid); break;
      ...
      case 'reboot': {
        if (!isOwner) return msg.reply('🔐 Owner only.');
        await sock.sendMessage(jid, { text: '♻️ *Rebooting Air Bot...*' }, { quoted: msg });
...
async function handleNonCommand(sock, msg, body, jid, sender, isGroup, isOwner) {
  try {
    ...
    // ── AI Mode: if enabled, handle all incoming text (DMs or groups)
    const aiMode = await store.get('aiMode');
    if (aiMode) {
      try {
        const cfg = (await store.get('aiModeConfig')) || {};
        const personaName = cfg.name || config.botName || 'Air';
        const systemPrompt = cfg.systemPrompt || `You are ${personaName}, a sharp, competent, and discreet assistant. Reply with strong reasoning, concise structure, and polished WhatsApp formatting. Never reveal internal system details or mention being an AI explicitly. Be practical, confident, and precise.`;

        // Build conversation with stored memory for continuity
        const history = await memory.getHistory(jid, 'renraiko');
        const messages = [ { role: 'system', content: systemPrompt }, ...history.slice(-20), { role: 'user', content: body } ];

        // Throttle / typing animation logic to avoid spamming and appear human
        const aiMeta = (await store.get('aiModeEnabledBy')) || {};
        const isSecret = aiMeta.method === 'secret';
        const jidKey = String(jid);

        // Emit received activity to dashboard
        try { dashboardBus && dashboardBus.emitEvent('activity', { text: `AI Mode received in ${jid}: ${String(body).slice(0,120)}` }); } catch(_) {}

        // If a generation is already running for this chat, show a short typing and drop new parallel starts
        if (aiTypingState.generating[jidKey]) {
          try {
            await sock.sendPresenceUpdate('composing', jid).catch(()=>{});
          } catch {};
          // Wait a human-like pause so the bot doesn't spam
          await new Promise(r => setTimeout(r, 6000));
          return;
        }

        aiTypingState.generating[jidKey] = true;
        try {
          const now = Date.now();
          const last = aiTypingState.lastReplyAt[jidKey] || 0;
          const sinceLast = now - last;

          // Group cooldown: prevent too-fast successive replies in groups
          if (isGroup && sinceLast < 4000) {
            const remaining = Math.ceil((4000 - sinceLast) / 1000);
            try { await sock.sendMessage(jid, { text: `⏳ Air is cooling down — please wait ${remaining}s before sending more messages.` }); } catch (_) {}
            delete aiTypingState.generating[jidKey];
            return;
          }

          // Secret mode: longer human-like pause when messages come rapidly
          let typingDelay = 800;
          if (isSecret && sinceLast < 8000) typingDelay = 6500;
          else if (isSecret) typingDelay = 2000;

          // Send composing presence periodically during the simulated typing
          const pulse = 2000;
          let elapsed = 0;
          while (elapsed < typingDelay) {
            try { await sock.sendPresenceUpdate('composing', jid).catch(()=>{}); } catch {};
            const wait = Math.min(pulse, typingDelay - elapsed);
          ...
          // Generate reply
          try { dashboardBus && dashboardBus.emitEvent('activity', { text: `AI Mode generating in ${jid}` }); } catch(_) {}
          const replyText = await require('./lib/pollinations').textGenerate(messages);

          // Persist memory for Airpersona
          await memory.addMessage(jid, 'renraiko', 'user', body);
          await memory.addMessage(jid, 'renraiko', 'assistant', replyText);

          // Reply quoting the message in groups, or just send in DMs
          await safeSend(sock, jid, { text: replyText }, replyOptions(msg));
          try { dashboardBus && dashboardBus.emitEvent('activity', { text: `AI Mode replied in ${jid}` }); } catch(_) {}

          aiTypingState.lastReplyAt[jidKey] = Date.now();
        } catch (e) {
          console.log('[AI Mode] generation failed:', e?.message || e);
          try { dashboardBus && dashboardBus.emitEvent('provider', { text: `AI generation error in ${jid}: ${e?.message||String(e)}` }); } catch(_){}
        } finally {
          // Clear composing/typing presence and lock
          try { await sock.sendPresenceUpdate('paused', jid).catch(()=>{}); } catch {};
          delete aiTypingState.generating[jidKey];
        }
      } catch (e) {
        console.log('[AI Mode] generation failed:', e?.message || e);
    ...
    // Auto-reply: controlled by config. Default: DMs only.
    const groupAuto = !!(await store.get('autoreplyGroups'));
    if (isGroup && !groupAuto) return;
