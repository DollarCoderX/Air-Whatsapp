...
// Emit activities to dashboard bus when available
try {
  const dashboardBus = require('./lib/dashboardBus');
  const oldLog = logActivity;
  logActivity = function(text) {
    oldLog(text);
    try { dashboardBus.emitEvent('activity', { text, at: Date.now() }); } catch(_){}
  };
  global.logBotActivity = logActivity;
} catch (_) {}

// Dashboard SSE clients and bus (used by the HTTP server below)
const sseClients = new Set();
const dashboardBus = (() => { try { return require('./lib/dashboardBus'); } catch { return null; } })();
if (dashboardBus) {
  dashboardBus.on('activity', (payload) => { for (const res of sseClients) try { res.write(`event: activity\ndata: ${JSON.stringify(payload)}\n\n`); } catch(_){} });
  dashboardBus.on('provider', (payload) => { for (const res of sseClients) try { res.write(`event: provider\ndata: ${JSON.stringify(payload)}\n\n`); } catch(_){} });
  dashboardBus.on('status', (payload) => { for (const res of sseClients) try { res.write(`event: status\ndata: ${JSON.stringify(payload)}\n\n`); } catch(_){} });
}
...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/dashboard/sse') {
    ...
    sseClients.add(res);
    // send initial status
    try { res.write(`event: status\ndata: ${JSON.stringify(global.botLiveStatus)}\n\n`); } catch(_){}
  ...
  if (req.url.startsWith('/dashboard')) {
    try {
      const filePath = req.url === '/dashboard' || req.url === '/dashboard/' ? '/index.html' : req.url.replace('/dashboard', '');
      const full = path.join(__dirname, 'dashboard', 'public', decodeURIComponent(filePath));
    ...
    } catch (e) { /* fallthrough to 404 below */ }
  }
...
<html><head><meta charset="utf-8"><title>Air</title>
...
<body>
  <div class="card">
    ...
    <div class="grid">
      ...
      <div class="stat"><div class="k">Engine</div><div class="v">AIR Intelligence</div></div>
    </div>
    <div class="feed">
      <h2>Live Activity</h2>
      <ul>${activityHtml}</ul>
...
function banner() {
  console.log('\n\x1b[33m╔══════════════════════════════╗\x1b[0m');
  console.log('\x1b[33m║  ✦  Air ✦  ║\x1b[0m');
  console.log('\x1b[33m╠══════════════════════════════╣\x1b[0m');
  console.log('\x1b[33m║\x1b[0m  Owner  : Dollar              \x1b[33m║\x1b[0m');
  console.log('\x1b[33m║\x1b[0m  Country: Canada 🇨🇦           \x1b[33m║\x1b[0m');
  console.log('\x1b[33m║\x1b[0m  Engine : AIR Intelligence    \x1b[33m║\x1b[0m');
  console.log('\x1b[33m║\x1b[0m  Version: 1.0                 \x1b[33m║\x1b[0m');
  console.log('\x1b[33m╚══════════════════════════════╝\x1b[0m\n');
}
...
async function startBot(method, phone) {
  ...
  sock.ev.on('connection.update', async update => {
    ...
    if (connection === 'open') {
      ...
      console.log('\x1b[32m║   AirONLINE!   ║\x1b[0m');
      console.log('\x1b[32m╚══════════════════════════════╝\x1b[0m\n');
      ...
      global.logBotActivity?.('Connected to WhatsApp — Air is online');
      try {
    ...
    for (const c of calls) {
      ...
      try {
        ...
        await sock.sendMessage(c.chatId || c.from, {
          text:
            `📵 *Aircan't take voice/video calls.*\n\n` +
            `WhatsApp calls aren't yet supported by Air— please send a text message instead and I'll get right back to you! For more information simply Dm +14378898269`,
        });
      } catch (e) {
