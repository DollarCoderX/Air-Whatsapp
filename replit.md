...
- `pnpm --filter @workspace/dollarbot run dev` — start the bot (via "Air V5" workflow)
...
Air V5 is a full-featured public WhatsApp bot that runs 24/7. Users interact with it by sending commands prefixed with `.` in any chat or group where the bot is active.
